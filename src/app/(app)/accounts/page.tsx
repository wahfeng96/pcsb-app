...
import { format, startOfMonth, endOfMonth, addMonths, subMonths, parseISO, isSameMonth, eachMonthOfInterval } from 'date-fns'
...
export default function AccountsPage() {
  ...
  useEffect(() => {
    async function load() {
      const [bb, bk, cs] = await Promise.all([
  ...
  // Per-billboard summary for selected month
  const billboardSummaries = useMemo(() => {
    return billboards.map(bb => {
      ...
      return {
        billboard: bb,
        monthRevenue, monthCost, monthProfit, partnerShare,
        totalRevenue, settledRevenue, pendingRevenue,
        activeThisMonth,
      }
    })
  ...
  function handleDownloadReport() {
    ...
    csv += `\n\nBooking Details - ${month}\n`
    csv += `Billboard,Client,Brand,Start,End,Monthly Rate (RM),Total (RM),Status,Payment,Sales Person\n`
    items.forEach(s => {
      s.activeThisMonth.forEach(b => {
        csv += `${s.billboard.name},${b.client?.company_name || ''},${b.brand_name || ''},${b.start_date},${b.end_date},${b.monthly_rate},${b.total_amount},${b.status},${b.payment_status},${b.sales_person || ''}\n`
      })
    })
      ...
      <Card>
        <CardContent className="p-4">
          ...
          <div className="flex rounded-lg overflow-hidden border text-center text-xs">
            <div className="flex-1 bg-green-50 p-2 border-r">
              <p className="text-[10px] text-green-700">Settled (All Time)</p>
              <p className="font-bold text-green-700">RM {grandTotals.settledRevenue.toLocaleString()}</p>
            </div>
            <div className="flex-1 bg-red-50 p-2">
              <p className="text-[10px] text-red-700">Pending</p>
              <p className="font-bold text-red-700">RM {grandTotals.pendingRevenue.toLocaleString()}</p>
            </div>
          </div>
        </CardContent>
      </Card>

      {/* Per-Billboard Breakdown */}
          ...
          <Card key={s.billboard.id}>
            <CardContent className="p-4">
              ...
              <div className="grid grid-cols-4 gap-1.5 text-xs mb-3">
                ...
                <div className={`rounded p-2 text-center ${s.monthProfit >= 0 ? 'bg-green-50' : 'bg-red-50'}`}>
              ...
              {/* Settled vs Pending */}
              <div className="flex rounded-lg overflow-hidden border text-center text-[10px] mb-3">
                <div className="flex-1 bg-green-50 p-1.5 border-r">
                  <p className="text-green-700">Settled</p>
                  <p className="font-bold text-green-700 text-xs">RM {s.settledRevenue.toLocaleString()}</p>
                </div>
                <div className="flex-1 bg-red-50 p-1.5">
                  <p className="text-red-700">Pending</p>
                  <p className="font-bold text-red-700 text-xs">RM {s.pendingRevenue.toLocaleString()}</p>
                </div>
              </div>
              ...
              {s.activeThisMonth.length > 0 && (
                <div className="space-y-1">
                  <p className="text-[10px] text-gray-400 font-medium">Active this month</p>
                  {s.activeThisMonth.map(b => (
                    <div key={b.id} className="flex items-center justify-between bg-gray-50 rounded px-2 py-1.5">
                      <div>
                        <span className="text-xs font-medium">{b.brand_name || b.client?.company_name}</span>
                        {b.sales_person && <span className="text-[10px] text-gray-400 ml-1">({b.sales_person})</span>}
                      </div>
                      <div className="text-right">
                        <p className="text-xs font-medium">RM {(b.monthly_rate || 0).toLocaleString()}/mo</p>
                        <Badge variant="outline" className={`text-[8px] ${b.payment_status === 'settled' ? 'text-green-600' : 'text-red-600'}`}>
                          {b.payment_status === 'settled' ? '✅ Settled' : b.payment_status === 'received_pending_profit_share' ? '⏳ Received' : '❌ Pending'}
                        </Badge>
                      </div>
                    </div>
                  ))}
