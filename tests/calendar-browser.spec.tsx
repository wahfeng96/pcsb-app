...
for (const mobile of [false, true]) {
  test(`calendar campaigns all display paths ${mobile ? 'mobile' : 'desktop'}`, async ({ mount, page }) => {
    ...
    const chip = page.locator('summary').filter({ hasText: /^Alpha \(in\) — Campaign: Launch$/ })
    await expect(chip).toHaveAttribute('title', 'Alpha (in) — Campaign: Launch')
    await expect(chip.locator('..').locator('..')).toHaveClass(/bg-green-500/)
    await chip.click()
    await expect(chip.locator('..')).toHaveAttribute('open', '')
    const out = page.locator('summary').filter({ hasText: /^Alpha \(out\) — Campaign: Launch$/ })
    ...
    const unsafe = page.locator('summary').filter({ hasText: /^Unsafe \(in\) — Campaign:/ })
    await expect(unsafe).toBeVisible()
    await unsafe.click()
    await expect(unsafe.locator('..').locator('span')).toHaveText('Unsafe (in) — Campaign: <img src=x onerror=alert(1)>')
    await expect(page.locator('img')).toHaveCount(0)
    await expect(page.getByText('Legacy (out)', { exact: true })).toBeVisible()
    await expect(page.getByText(/Cancelled/)).toHaveCount(0)
    await expect(page.locator('summary').filter({ hasText: /^Alpha — Campaign: Launch$/ })).toHaveCount(2)
    ...
    await expect(tooltip).toContainText('Alpha — Campaign: Launch')
    await expect(tooltip).toContainText(`Beta — Campaign: ${'X'.repeat(200)}`)
